# -*- coding: utf-8 -*-
# Generated by Django 1.11.4 on 2017-10-14 23:36
from __future__ import unicode_literals

from django.db import migrations, models
import django.db.models.deletion


class Migration(migrations.Migration):

    dependencies = [
        ('users', '0001_initial'),
    ]

    operations = [
        migrations.CreateModel(
            name='CTImages',
            fields=[
                ('id', models.AutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('SOPInstanceUID', models.CharField(max_length=200)),
                ('SOPClassUID', models.CharField(max_length=100)),
                ('ImageType', models.CharField(max_length=20)),
                ('PhotometriInterpretation', models.CharField(max_length=20, null=True)),
                ('RescaleSlope', models.IntegerField()),
                ('RescaleIntercept', models.IntegerField()),
                ('SliceLocation', models.IntegerField()),
                ('PixelSpacing', models.CharField(max_length=50)),
                ('ImageOrientationPatient', models.CharField(max_length=20)),
                ('ImagePositionPatient', models.CharField(max_length=20)),
                ('SliceThickness', models.CharField(max_length=50)),
                ('BodypartExamined', models.CharField(max_length=50)),
                ('Rows', models.IntegerField()),
                ('Columns', models.IntegerField()),
            ],
            options={
                'db_table': 'ct_images',
            },
        ),
        migrations.CreateModel(
            name='Patient',
            fields=[
                ('id', models.AutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('PatientName', models.CharField(max_length=200)),
                ('BirthDate', models.DateField(null=True)),
                ('Gender', models.CharField(max_length=20, null=True)),
                ('EnthicGroup', models.CharField(max_length=200, null=True)),
                ('fk_user_id', models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to='users.User')),
            ],
            options={
                'db_table': 'patients',
            },
        ),
        migrations.CreateModel(
            name='RTContour',
            fields=[
                ('id', models.AutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('ContourGeometricType', models.CharField(max_length=100)),
                ('NumberOfContourPoints', models.IntegerField()),
                ('ContourData', models.TextField()),
                ('ReferencedSOPClassUID', models.CharField(max_length=100)),
                ('ReferencedSOPInstanceUID', models.CharField(max_length=100)),
            ],
            options={
                'db_table': 'rt_contour',
            },
        ),
        migrations.CreateModel(
            name='RTDose',
            fields=[
                ('id', models.AutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('SOPClassUID', models.CharField(max_length=100)),
                ('SOPInstanceUID', models.CharField(max_length=100)),
                ('DoseGridScaling', models.CharField(max_length=50)),
                ('DoseSummationType', models.CharField(max_length=50, null=True)),
                ('DoseType', models.CharField(max_length=20, null=True)),
                ('DoseUnits', models.CharField(max_length=10, null=True)),
                ('ReferencedRTPlanSequence', models.CharField(max_length=100)),
                ('ReferencedStructureSetSequence', models.CharField(max_length=100)),
                ('fk_patient_id', models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to='users.Patient')),
            ],
            options={
                'db_table': 'rt_dose',
            },
        ),
        migrations.CreateModel(
            name='RTDoseImage',
            fields=[
                ('id', models.AutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('Columns', models.IntegerField()),
                ('Rows', models.IntegerField()),
                ('ImageOrientationPatient', models.CharField(max_length=20)),
                ('ImagePositionPatient', models.CharField(max_length=20)),
                ('PhotometricInterpretation', models.CharField(max_length=20, null=True)),
                ('PixelSpacing', models.CharField(max_length=20)),
                ('SliceThickness', models.CharField(max_length=10, null=True)),
                ('fk_dose_id', models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to='users.RTDose')),
                ('fk_patient_id', models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to='users.Patient')),
            ],
            options={
                'db_table': 'rt_dose_image',
            },
        ),
        migrations.CreateModel(
            name='RTDVH',
            fields=[
                ('id', models.AutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('DVHDoseScaling', models.CharField(max_length=20)),
                ('DVHMaximumDose', models.FloatField()),
                ('DVHMeanDose', models.FloatField()),
                ('DVHMinimumDose', models.FloatField()),
                ('DVHNumberOfBins', models.IntegerField()),
                ('DVHReferencedROI', models.CharField(max_length=10)),
                ('DVHType', models.CharField(max_length=10, null=True)),
                ('DVHVolumeUnits', models.CharField(max_length=10)),
                ('DoseType', models.CharField(max_length=10)),
                ('DoseUnits', models.CharField(max_length=10)),
                ('DVHData', models.TextField()),
                ('fk_dose_id', models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to='users.RTDose')),
                ('fk_patient_id', models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to='users.Patient')),
            ],
            options={
                'db_table': 'rt_dvh',
            },
        ),
        migrations.CreateModel(
            name='RTIsDose',
            fields=[
                ('id', models.AutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('RowPosition', models.TextField()),
                ('ColumnPosition', models.TextField()),
                ('IsDoseValue', models.IntegerField()),
                ('fk_ct_image_id', models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to='users.CTImages')),
                ('fk_dose_id', models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to='users.RTDose')),
                ('fk_patient_id', models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to='users.Patient')),
            ],
            options={
                'db_table': 'rt_isdose',
            },
        ),
        migrations.CreateModel(
            name='RTROI',
            fields=[
                ('id', models.AutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('ROIName', models.CharField(max_length=100)),
                ('ROIDisplayColor', models.CharField(max_length=50)),
                ('ROINumber', models.CharField(max_length=10)),
                ('Volume', models.FloatField()),
                ('TotalContours', models.IntegerField()),
            ],
            options={
                'db_table': 'rt_rois',
            },
        ),
        migrations.CreateModel(
            name='RTStructureSet',
            fields=[
                ('id', models.AutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('SOPInstanceUID', models.CharField(max_length=200)),
                ('SOPClassUID', models.CharField(max_length=100)),
                ('TotalROIs', models.IntegerField()),
                ('fk_patient_id', models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to='users.Patient')),
            ],
            options={
                'db_table': 'rt_structureset',
            },
        ),
        migrations.CreateModel(
            name='Series',
            fields=[
                ('id', models.AutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('SeriesInstanceUID', models.CharField(max_length=200)),
                ('SeriesDate', models.DateField()),
                ('SeriesDescription', models.CharField(max_length=200, null=True)),
                ('SeriesType', models.CharField(max_length=100)),
                ('Modality', models.CharField(max_length=100)),
                ('SeriesNumber', models.CharField(max_length=100, null=True)),
                ('PhysicianOfRecord', models.CharField(max_length=100, null=True)),
                ('Manufacturer', models.CharField(max_length=50, null=True)),
                ('fk_patient_id', models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to='users.Patient')),
            ],
            options={
                'db_table': 'series',
            },
        ),
        migrations.CreateModel(
            name='Study',
            fields=[
                ('id', models.AutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('StudyInstanceUID', models.CharField(max_length=200)),
                ('StudyDate', models.DateField(null=True)),
                ('StudyDescription', models.CharField(max_length=200, null=True)),
                ('TotalSeries', models.IntegerField()),
                ('fk_patient_id', models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to='users.Patient')),
                ('fk_user_id', models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to='users.User')),
            ],
            options={
                'db_table': 'studies',
            },
        ),
        migrations.AddField(
            model_name='series',
            name='fk_study_id',
            field=models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to='users.Study'),
        ),
        migrations.AddField(
            model_name='series',
            name='fk_user_id',
            field=models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to='users.User'),
        ),
        migrations.AddField(
            model_name='rtstructureset',
            name='fk_series_id',
            field=models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to='users.Series'),
        ),
        migrations.AddField(
            model_name='rtstructureset',
            name='fk_study_id',
            field=models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to='users.Study'),
        ),
        migrations.AddField(
            model_name='rtstructureset',
            name='fk_user_id',
            field=models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to='users.User'),
        ),
        migrations.AddField(
            model_name='rtroi',
            name='fk_structureset_id',
            field=models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to='users.RTStructureSet'),
        ),
        migrations.AddField(
            model_name='rtisdose',
            name='fk_series_id',
            field=models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to='users.Series'),
        ),
        migrations.AddField(
            model_name='rtisdose',
            name='fk_study_id',
            field=models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to='users.Study'),
        ),
        migrations.AddField(
            model_name='rtisdose',
            name='fk_user_id',
            field=models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to='users.User'),
        ),
        migrations.AddField(
            model_name='rtdvh',
            name='fk_series_id',
            field=models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to='users.Series'),
        ),
        migrations.AddField(
            model_name='rtdvh',
            name='fk_study_id',
            field=models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to='users.Study'),
        ),
        migrations.AddField(
            model_name='rtdvh',
            name='fk_user_id',
            field=models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to='users.User'),
        ),
        migrations.AddField(
            model_name='rtdoseimage',
            name='fk_series_id',
            field=models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to='users.Series'),
        ),
        migrations.AddField(
            model_name='rtdoseimage',
            name='fk_study_id',
            field=models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to='users.Study'),
        ),
        migrations.AddField(
            model_name='rtdoseimage',
            name='fk_user_id',
            field=models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to='users.User'),
        ),
        migrations.AddField(
            model_name='rtdose',
            name='fk_series_id',
            field=models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to='users.Series'),
        ),
        migrations.AddField(
            model_name='rtdose',
            name='fk_study_id',
            field=models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to='users.Study'),
        ),
        migrations.AddField(
            model_name='rtdose',
            name='fk_user_id',
            field=models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to='users.User'),
        ),
        migrations.AddField(
            model_name='rtcontour',
            name='fk_roi_id',
            field=models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to='users.RTROI'),
        ),
        migrations.AddField(
            model_name='rtcontour',
            name='fk_structureset_id',
            field=models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to='users.RTStructureSet'),
        ),
        migrations.AddField(
            model_name='ctimages',
            name='fk_patient_id',
            field=models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to='users.Patient'),
        ),
        migrations.AddField(
            model_name='ctimages',
            name='fk_series_id',
            field=models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to='users.Series'),
        ),
        migrations.AddField(
            model_name='ctimages',
            name='fk_study_id',
            field=models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to='users.Study'),
        ),
        migrations.AddField(
            model_name='ctimages',
            name='fk_user_id',
            field=models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to='users.User'),
        ),
    ]
