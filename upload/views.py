...
def processUploadedFile(raw_file_path,patientName,user):
    ...
    rootDir = os.path.join(STATIC_ROOT, 'data',str(user.userid),patientName)  # set the directory you want to start from
...
def uploadForm(request):
    ...
    if request.method == 'POST':  # check whether a form has been submitted
        ...
        if res:
            return HttpResponse("parse successfully")
        else:
            return HttpResponse("Fail to process the file")
    else:
